...
/*
 * The nFBTweakEXT few macros create type definitions for the unions used for each
...
#define ADT_equalto_iter(INDEX, CONS, PARAM) \
    { \
        const char *paramEncoding = FBTweakEXT_trimADTJunkFromTypeEncoding(@encode(ADT_CURRENT_CONS_ALIAS_T(CONS, INDEX)); \
        \
...
#define ADT_CURRENT_PARAMETER_DESCRIPTION(CONS, PARAM, ADT, INDEX) \
    [NSString stringWithFormat:@"%@ = %@", \
        ...
        FBTweakEXT_parameterNameFromDeclaration(@ # PARAM), \
        ...
        FBTweakEXT_stringFromTypedBytes(&(ADT).ADT_CURRENT_CONS_PAYLOAD_T(CONS, INDEX), \
            FBTweakEXT_trimADTJunkFromTypeEncoding(@encode(ADT_CURRENT_CONS_ALIAS_T(CONS, INDEX)))) \
    ]
