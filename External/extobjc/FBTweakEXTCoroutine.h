...
#define coroutine(...) \
    ^{ \
        __block unsigned long FBTweakEXT_coroutine_line_ = 0; \
        \
        return [ \
 ...
 */
#define yield \
    if ((FBTweakEXT_coroutine_line_ = __LINE__) == 0) \
        case __LINE__: \
            ; \
    else \
        return
...
#define coroutine_body(STATEMENT) \
            { \
                for (;; FBTweakEXT_coroutine_line_ = 0) \
                    switch (FBTweakEXT_coroutine_line_) \
                        default: \
                            STATEMENT \
            } \
        copy]; \
