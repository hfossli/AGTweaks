...
typedef void (^FBTweakEXT_specialProtocolInjectionBlock)(Class);

// a `const char *` equivalent to system struct objc_method_description
typedef struct {
    SEL name;
    const char *types;
} FBTweakEXT_methodDescription;
...
typedef struct {
    ...
    BOOL ready;
} FBTweakEXTSpecialProtocol;

// the full list of special protocols (an array of FBTweakEXTSpecialProtocol structs)
static FBTweakEXTSpecialProtocol * restrict specialProtocols = NULL;
...
// the number of FBTweakEXTSpecialProtocols which have been marked as ready for
 ...
 */
static void FBTweakEXT_injectSpecialProtocols (void) {
    /*
     ...
     */
    qsort_b(specialProtocols, specialProtocolCount, sizeof(FBTweakEXTSpecialProtocol), ^(const void *a, const void *b){
        // if the pointers are equal, it must be the same protocol
        if (a == b)
            return 0;

        const FBTweakEXTSpecialProtocol *protoA = a;
        const FBTweakEXTSpecialProtocol *protoB = b;

        // A higher return value here means a higher priority
        int (^protocolInjectionPriority)(const FBTweakEXTSpecialProtocol *) = ^(const FBTweakEXTSpecialProtocol *specialProtocol){
            int runningTotal = 0;
	...
	// use this instead of FBTweakEXT_copyClassList() to avoid sending +initialize to
    ...
    @autoreleasepool {
        ...
        for (size_t i = 0;i < specialProtocolCount;++i) {
            ...
            // from the structure
            FBTweakEXT_specialProtocolInjectionBlock injectionBlock = (__bridge_transfer id)specialProtocols[i].injectionBlock;
...
unsigned FBTweakEXT_injectMethods (
    Class aClass,
    Method *methods,
    unsigned count,
    FBTweakEXT_methodInjectionBehavior behavior,
    FBTweakEXT_failedMethodCallback failedToAddCallback
) {
    ...
    @autoreleasepool {
        ...
        if (!isMeta) {
            ...
            behavior &= ~(FBTweakEXT_methodInjectionIgnoreLoad | FBTweakEXT_methodInjectionIgnoreInitialize);
        }
        ...
        for (unsigned methodIndex = 0;methodIndex < count;++methodIndex) {
            ...
            if (behavior & FBTweakEXT_methodInjectionIgnoreLoad) {
            ...
            if (behavior & FBTweakEXT_methodInjectionIgnoreInitialize) {
            ...
            switch (behavior & FBTweakEXT_methodInjectionOverwriteBehaviorMask) {
            case FBTweakEXT_methodInjectionFailOnExisting:
                success = class_addMethod(aClass, methodName, impl, type);
                break;

            case FBTweakEXT_methodInjectionFailOnAnyExisting:
            ...
            case FBTweakEXT_methodInjectionReplace:
                class_replaceMethod(aClass, methodName, impl, type);
                break;

            case FBTweakEXT_methodInjectionFailOnSuperclassExisting:
                {
...
BOOL FBTweakEXT_injectMethodsFromClass (
    Class srcClass,
    Class dstClass,
    FBTweakEXT_methodInjectionBehavior behavior,
    FBTweakEXT_failedMethodCallback failedToAddCallback)
{
    ...
    addedCount = FBTweakEXT_injectMethods(
        dstClass,
        instanceMethods,
        count,
        behavior,
    ...
    // ignore +load
    behavior |= FBTweakEXT_methodInjectionIgnoreLoad;
    addedCount = FBTweakEXT_injectMethods(
...
Class FBTweakEXT_classBeforeSuperclass (Class receiver, Class superclass) {
...
Class *FBTweakEXT_copyClassList (unsigned *count) {
...
unsigned FBTweakEXT_addMethods (Class aClass, Method *methods, unsigned count, BOOL checkSuperclasses, FBTweakEXT_failedMethodCallback failedToAddCallback) {
    FBTweakEXT_methodInjectionBehavior behavior = FBTweakEXT_methodInjectionFailOnExisting;
    if (checkSuperclasses)
        behavior |= FBTweakEXT_methodInjectionFailOnSuperclassExisting;

    return FBTweakEXT_injectMethods(
        aClass,
        methods,
        count,
        behavior,
...
BOOL FBTweakEXT_addMethodsFromClass (Class srcClass, Class dstClass, BOOL checkSuperclasses, FBTweakEXT_failedMethodCallback failedToAddCallback) {
    FBTweakEXT_methodInjectionBehavior behavior = FBTweakEXT_methodInjectionFailOnExisting;
    if (checkSuperclasses)
        behavior |= FBTweakEXT_methodInjectionFailOnSuperclassExisting;
    
    return FBTweakEXT_injectMethodsFromClass(srcClass, dstClass, behavior, failedToAddCallback);
}

BOOL FBTweakEXT_classIsKindOfClass (Class receiver, Class aClass) {
    while (receiver) {
        if (receiver == aClass)
            return YES;
...
Class *FBTweakEXT_copyClassListConformingToProtocol (Protocol *protocol, unsigned *count) {
    Class *allClasses;
    ...
    @autoreleasepool {
        ...
        allClasses = FBTweakEXT_copyClassList(&classCount);
...
FBTweakEXT_propertyAttributes *FBTweakEXT_copyPropertyAttributes (objc_property_t property) {
    ...
    const char *nFBTweakEXT = NSGetSizeAndAlignment(typeString, NULL, NULL);
    if (!nFBTweakEXT) {
        fprintf(stderr, "ERROR: Could not read past type in attribute string \"%s\" for property %s\n", attrString, property_getName(property));
        return NULL;
    }

    size_t typeLength = nFBTweakEXT - typeString;
    if (!typeLength) {
    ...
    FBTweakEXT_propertyAttributes *attributes = calloc(1, sizeof(FBTweakEXT_propertyAttributes) + typeLength + 1);
    if (!attributes) {
        fprintf(stderr, "ERROR: Could not allocate FBTweakEXT_propertyAttributes structure for attribute string \"%s\" for property %s\n", attrString, property_getName(property));
        return NULL;
    }
    ...
    if (typeString[0] == *(@encode(id)) && typeString[1] == '"') {
        // we should be able to FBTweakEXTract a class name
        const char *className = typeString + 2;
        nFBTweakEXT = strchr(className, '"');
        ...
        if (className != nFBTweakEXT) {
            size_t classNameLength = nFBTweakEXT - className;
    ...
    if (*nFBTweakEXT != '\0') {
        ...
        nFBTweakEXT = strchr(nFBTweakEXT, ',');
    }

    while (nFBTweakEXT && *nFBTweakEXT == ',') {
        char flag = nFBTweakEXT[1];
        nFBTweakEXT += 2;
        ...
        case 'C':
            attributes->memoryManagementPolicy = FBTweakEXT_propertyMemoryManagementPolicyCopy;
            break;

        case '&':
            attributes->memoryManagementPolicy = FBTweakEXT_propertyMemoryManagementPolicyRetain;
            break;
        ...
        case 'S':
            {
                const char *nFBTweakEXTFlag = strchr(nFBTweakEXT, ',');
                SEL name = NULL;

                if (!nFBTweakEXTFlag) {
                    // assume that the rest of the string is the selector
                    const char *selectorString = nFBTweakEXT;
                    nFBTweakEXT = "";

                    name = sel_registerName(selectorString);
                } else {
                    size_t selectorLength = nFBTweakEXTFlag - nFBTweakEXT;
                    if (!selectorLength) {
                    ...
                    strncpy(selectorString, nFBTweakEXT, selectorLength);
                    selectorString[selectorLength] = '\0';

                    name = sel_registerName(selectorString);
                    nFBTweakEXT = nFBTweakEXTFlag;
                }
        ...
        case 't':
            ...
            while (*nFBTweakEXT != ',' && *nFBTweakEXT != '\0')
                ++nFBTweakEXT;
    ...
    if (nFBTweakEXT && *nFBTweakEXT != '\0') {
        fprintf(stderr, "Warning: Unparsed data \"%s\" in attribute string \"%s\" for property %s\n", nFBTweakEXT, attrString, property_getName(property));
    }
...
Class *FBTweakEXT_copySubclassList (Class targetClass, unsigned *subclassCount) {
    unsigned classCount = 0;
    Class *allClasses = FBTweakEXT_copyClassList(&classCount);
...
Method FBTweakEXT_getImmediateInstanceMethod (Class aClass, SEL aSelector) {
...
BOOL FBTweakEXT_getPropertyAccessorsForClass (objc_property_t property, Class aClass, Method *getter, Method *setter) {
...
NSMethodSignature *FBTweakEXT_globalMethodSignatureForSelector (SEL aSelector) {
    ...
    static FBTweakEXT_methodDescription volatile methodDescriptionCache[selectorCacheLength];
    ...
    FBTweakEXT_methodDescription methodDesc;
    ...
    methodDesc = (FBTweakEXT_methodDescription){.name = NULL, .types = NULL};

    uint classCount = 0;
    Class *classes = FBTweakEXT_copyClassList(&classCount);
    ...
    if (classes) {
        @autoreleasepool {
            ...
            for (uint i = 0;i < classCount;++i) {
                ...
                if (method) {
                    methodDesc = (FBTweakEXT_methodDescription){.name = aSelector, .types = method_getTypeEncoding(method)};
                    break;
                }
            }
        }
    ...
    if (!methodDesc.name) {
        ...
        if (protocols) {
            ...
            for (uint i = 0;i < protocolCount;++i) {
                ...
                if (objcMethodDesc.name) {
                    methodDesc = (FBTweakEXT_methodDescription){.name = objcMethodDesc.name, .types = objcMethodDesc.types};
                    break;
                }
            }
            free(protocols);
...
BOOL FBTweakEXT_loadSpecialProtocol (Protocol *protocol, void (^injectionBehavior)(Class destinationClass)) {
    @autoreleasepool {
        ...
        // FBTweakEXT_injectSpecialProtocols()
        #ifndef __clang_analyzer__
        FBTweakEXT_specialProtocolInjectionBlock copiedBlock = [injectionBehavior copy];

        // construct a new FBTweakEXTSpecialProtocol structure and add it to the first
        // empty space in the array
        specialProtocols[specialProtocolCount] = (FBTweakEXTSpecialProtocol){
            .protocol = protocol,
...
void FBTweakEXT_specialProtocolReadyForInjection (Protocol *protocol) {
    @autoreleasepool {
        NSCParameterAssert(protocol != nil);
...
void FBTweakEXT_removeMethod (Class aClass, SEL methodName) {
    Method existingMethod = FBTweakEXT_getImmediateInstanceMethod(aClass, methodName);
    if (!existingMethod) {
        return;
    }
...
void FBTweakEXT_replaceMethods (Class aClass, Method *methods, unsigned count) {
    FBTweakEXT_injectMethods(
        aClass,
        methods,
        count,
        FBTweakEXT_methodInjectionReplace,
        NULL
    );
}

void FBTweakEXT_replaceMethodsFromClass (Class srcClass, Class dstClass) {
    FBTweakEXT_injectMethodsFromClass(srcClass, dstClass, FBTweakEXT_methodInjectionReplace, NULL);
}

NSString *FBTweakEXT_stringFromTypedBytes (const void *bytes, const char *encoding) {
    switch (*encoding) {
