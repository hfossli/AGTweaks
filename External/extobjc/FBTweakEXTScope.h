 ...
 */
#define onExit \
    FBTweakEXT_keywordify \
    __strong FBTweakEXT_cleanupBlock_t metamacro_concat(FBTweakEXT_exitBlock_, __LINE__) __attribute__((cleanup(FBTweakEXT_executeCleanupBlock), unused)) = ^
...
#define weakify(...) \
    FBTweakEXT_keywordify \
    metamacro_foreach_cxt(FBTweakEXT_weakify_,, __weak, __VA_ARGS__)
...
#define unsafeify(...) \
    FBTweakEXT_keywordify \
    metamacro_foreach_cxt(FBTweakEXT_weakify_,, __unsafe_unretained, __VA_ARGS__)
...
#define strongify(...) \
    ...
    metamacro_foreach(FBTweakEXT_strongify_,, __VA_ARGS__) \
    _Pragma("clang diagnostic pop")

/*** implementation details follow ***/
typedef void (^FBTweakEXT_cleanupBlock_t)();

void FBTweakEXT_executeCleanupBlock (__strong FBTweakEXT_cleanupBlock_t *block);

#define FBTweakEXT_weakify_(INDEX, CONTEXT, VAR) \
    CONTEXT __typeof__(VAR) metamacro_concat(VAR, _weak_) = (VAR);

#define FBTweakEXT_strongify_(INDEX, VAR) \
...
// autorelease pools.
#if DEBUG
#define FBTweakEXT_keywordify autoreleasepool {}
#else
#define FBTweakEXT_keywordify try {} @catch (...) {}
#endif
