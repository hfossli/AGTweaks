...
#define checkselector(TARGET, ...) \
    (((void)(NO && ((void)[TARGET metamacro_foreach(FBTweakEXT_checkselector_message_iter,, __VA_ARGS__)], NO)), \
        metamacro_foreach(FBTweakEXT_checkselector_selector_iter,, __VA_ARGS__))).FBTweakEXT_toSelector

#define checkselector0(TARGET, SELECTOR) \
    (((void)(NO && ((void)[TARGET SELECTOR], NO)), \
        # SELECTOR)).FBTweakEXT_toSelector

/*** implementation details follow ***/
#define FBTweakEXT_checkselector_message_iter(INDEX, SELPART) \
    SELPART (0)

#define FBTweakEXT_checkselector_selector_iter(INDEX, SELPART) \
    # SELPART

@interface NSString (FBTweakEXTCheckedSelectorAdditions)
