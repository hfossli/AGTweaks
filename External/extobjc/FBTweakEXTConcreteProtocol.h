...
#define concreteprotocol(NAME) \
    ...
    + (void)load { \
         ...
         * the second
         */ \
        if (!FBTweakEXT_addConcreteProtocol(objc_getProtocol(metamacro_stringify(NAME)), self)) \
    ...
    static void FBTweakEXT_ ## NAME ## _inject (void) { \
        /*
         * use this injection point to mark this concrete protocol as ready for
         * loading
         */ \
        FBTweakEXT_loadConcreteProtocol(objc_getProtocol(metamacro_stringify(NAME))); \
    }

/*** implementation details follow ***/
BOOL FBTweakEXT_addConcreteProtocol (Protocol *protocol, Class methodContainer);
