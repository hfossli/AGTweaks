 ...
 * ways. If \a CLASS conforms to a protocol which has an FBTweakEXTProtocolCategory,
...
#define safecategory(CLASS, CATEGORY) \
    ...
    static void FBTweakEXT_ ## CLASS ## _ ## CATEGORY ## _inject (void) { \
        /*
         ...
         * loaded already (though the interaction with FBTweakEXTProtocolCategory is
         * undefined)
         */ \
         ...
         * (using FBTweakEXT_safeCategoryFailed)
         */ \
        if (!FBTweakEXT_loadSafeCategory(methodContainer_, targetClass_)) {\
            FBTweakEXT_safeCategoryFailed(CLASS, CATEGORY); \
        } \
    }

/*** implementation details follow ***/
BOOL FBTweakEXT_loadSafeCategory (Class methodContainer, Class targetClass);

// if this is a debug build...
#if defined(DEBUG) && !defined(NDEBUG)
    // abort if a safe category fails to load
    #define FBTweakEXT_safeCategoryFailed(CLASS, CATEGORY) \
        do { \
